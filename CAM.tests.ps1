...
if (!$CloudBuild) {
    Describe "New-CAMSchedule" {
        if ("$((Get-Item -Path ".\").FullName)\CAM.psm1") {
           $path = "$((Get-Item -Path ".\").FullName)\localManifest.json"
           ...
           It "Creates a scheduled task with provided time frequency" {
                New-CamSchedule -Frequency 10
                $task = Get-ScheduledTask -TaskName "CAM" | Should -Be $true
                (Get-ScheduledTask -TaskName "CAM").Triggers.Repetition.Interval | Should -Be 'PT10M'
                Unregister-ScheduledTask -TaskName "CAM" -Confirm:$false
           }
           It "Creates a scheduled task with provided local manifest" {
               New-CamSchedule -LocalManifest $Path
               $task = Get-ScheduledTask | ?{ $_.TaskName -eq "CAM" } | Should -Be $true
               (Get-ScheduledTask -TaskName "CAM").Actions.Arguments | Should -Be "Import-Module .\CAM.psm1; Install-KVCertificates -LocalManifest ""$Path"""
               Unregister-ScheduledTask -TaskName "CAM" -Confirm:$false
           }
        }
    }
...
If (!$CloudBuild) {
    ...
    Describe "Install-KVCertificates" {
        ...
        if (test-path ($Path -replace "\\CAMConfig.json", "\\localManifest.json")) {
            ...
            It "Installs certificates from different Key Vaults" {
